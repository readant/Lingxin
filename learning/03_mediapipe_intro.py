...
姿态关键点（上半身15个）
-----------------------
本项目取 MediaPipe Pose Landmarks 标准 33 点中的前 15 个（索引 0-14），
即头部 + 双臂，与 src/constants.py 中 POSE_CONNECTIONS_HOLISTIC 的约定一致。
索引 | 位置
-----|------------
0    | 鼻子
1    | 左眼内眼角
...
注意：MediaPipe 原生返回 33 个全身关键点，本项目只用上半身 15 个（索引 0-14）。
具体见 src/detection/hand_detector.py 的 PoseDetector。
